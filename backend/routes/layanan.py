...
def create():
    d = request.get_json()
    lid = execute(
        """INSERT INTO layanan (lay_nama, lay_harga, satuan, estimasi_hari, icon, id_jenis_barang)
           VALUES (%s, %s, %s, %s, %s, %s)""",
        (d["lay_nama"], d["lay_harga"], d["satuan"], d.get("estimasi_hari", 0), d.get("icon", "🧺"), d["id_jenis_barang"])
    )
...
def update(id):
    ...
    execute(
        """UPDATE layanan SET lay_nama=%s, lay_harga=%s, satuan=%s,
           estimasi_hari=%s, icon=%s, id_jenis_barang=%s WHERE id_layanan=%s""",
        (d["lay_nama"], d["lay_harga"], d["satuan"], d.get("estimasi_hari", 0), d.get("icon", "🧺"), d["id_jenis_barang"], id)
    )
